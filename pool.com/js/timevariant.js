...
var payoutarr=new Array("week","month","year");
function changePayoutTab(period)
{
	var current_period=$("#payout-navbar > li.active > a")[0].innerHTML;
	if(period==current_period.toLowerCase()) return;
	else
	{
		$("#payout-navbar > li.active").removeClass("active");
		var index=payoutarr.indexOf(period);
		$("#payout-navbar > li:eq("+index+")").addClass("active");
		getPayout(id,period);
	}
}
...
function drawHashChart(inputData,period) {
	...
	else if(period=="month") format="d MMM";
...
function getPayout(id,period)
{
	...
	$.ajax({
		type: "POST",
		dataType: "json",
		url: "http://reassent.no-ip.biz:8088/pool.com/payout-chart.php",
